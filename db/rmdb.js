...
async function getGitPRs(loginD, parms) {
    ...
    if (repoGroupVal) {
        ...
        getRepoPRArray.forEach(async function (repo, idx) {
            console.log("EACHREPO: " + JSON.stringify(repo));
            gitPRs[repo.repo_name] = await getGitPRs2(loginD, parms, getRepoPRArray[repo]);
        ...
        await axios({
            ...
            .then(function (resp) {
                ...
                if (resp.data.data.search.issueCount > 0) {
                    gitPRs[rpoObject.repo_name + "_page" + counter] = resp.data;
                    //console.log('GITPRS: ' + JSON.stringify(gitPRs));
